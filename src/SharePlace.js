...
import { getAddressFromCoords, getCoordsFromAddress} from './Utitlity/Location';
 ...
 class PlaceFinder{
     ...
     locateUserHandler(){
         if(!navigator.geolocation){
             alert('Location feature is not available in your browser. Please enter a valid address or use modern browser');
             return
         }
         ...
         navigator.geolocation.getCurrentPosition(async successResult=>{
             modal.hide()
            const coordinates = {
                lat: parseFloat(successResult.coords.latitude),
                lng: parseFloat(successResult.coords.longitude)
            };
    ...
    async findAddressHandler(){
         ...
         if(!address||address.trim().length===0){
             alert('Please add a valied address');
             return 
         }
         const modal = new Modal('loading-modal-content', 'Loading location - please wait');
